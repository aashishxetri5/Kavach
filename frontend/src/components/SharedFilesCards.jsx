...
const SharedFilesCards = ({ userData }) => {
  const { sharedBy, files } = userData;

  return (
    <div className="w-full">
      <div className="mt-4">
        <h5 className="sharedByName mb-0">{sharedBy.fullname}</h5>
        <span className="sharedByEmail block text-xs">{sharedBy.email}</span>
      </div>
